@extends('layouts.app')
@section('title',trans('tests.tests'))
@section('header')
                      ...
                      <div class="row">
                        @foreach($tests as $Test)
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title">#Code {{$Test->id}}</h5>
                                        <h6 class="card-subtitle mb-2 text-muted">Card subtitle</h6>
                                        <p class="card-text">
                                            Some quick example text to build on the card title and make
                                            up the bulk of the cards content.</p>
                                        <a class="btn btn-primary" href="{{ url('tests/'.$Test->id) }}" class="card-link">view</a>
                                        <a class="btn btn-success" href="{{ url('tests/'.$Test->id.'/edit') }}" class="card-link">update</a>

                                       <form id="{{$Test->id}}" method="POST"
                                       action="{{url('tests/'.$Test->id)}}"
                                       style="display:inline">
                                        <input name="_method" type="hidden" value="DELETE">
                                        <input name="_token" type="hidden" value="{{csrf_token()}}">
                                        <button type="button" onclick="return deleteItem('{{$Test->id}}')"
