                      ...
                      <div class="row">
                        @foreach($abtests as $Abtest)
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title">#Code {{$Abtest->id}}</h5>
                                        <h6 class="card-subtitle mb-2 text-muted">Card subtitle</h6>
                                        <p class="card-text">
                                            Some quick example text to build on the card title and make
                                            up the bulk of the cards content.</p>
                                        <a class="btn btn-primary" href="{{ url('abtests/'.$Abtest->id) }}" class="card-link">view</a>
                                        <a class="btn btn-success" href="{{ url('abtests/'.$Abtest->id.'/edit') }}" class="card-link">update</a>

                                       <form id="{{$Abtest->id}}" method="POST"
                                       action="{{url('abtests/'.$Abtest->id)}}"
                                       style="display:inline">
                                        <input name="_method" type="hidden" value="DELETE">
                                        <input name="_token" type="hidden" value="{{csrf_token()}}">
                                        <button type="button" onclick="return deleteItem('{{$Abtest->id}}')"
