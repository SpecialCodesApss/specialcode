...
<div class="pcoded-inner-content">
    ...
    <div class="wrapper_cols">
        <div class="col_page_content">
            ...
            href="{{ url('admin_messages/create') }}">
                      ...
                      <div class="row">
                        @foreach($admin_messages as $Admin_message)
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title">#Code {{$Admin_message->id}}</h5>
                                        <h6 class="card-subtitle mb-2 text-muted">Card subtitle</h6>
                                        <p class="card-text">
                                            Some quick example text to build on the card title and make
                                            up the bulk of the cards content.</p>
                                        <a class="btn btn-primary" href="{{ url('admin_messages/'.$Admin_message->id) }}" class="card-link">view</a>
                                        <a class="btn btn-success" href="{{ url('admin_messages/'.$Admin_message->id.'/edit') }}" class="card-link">update</a>

                                       <form id="{{$Admin_message->id}}" method="POST"
                                       action="{{url('admin_messages/'.$Admin_message->id)}}"
                                       style="display:inline">
                                        <input name="_method" type="hidden" value="DELETE">
                                        <input name="_token" type="hidden" value="{{csrf_token()}}">
                                        <button type="button" onclick="return deleteItem('{{$Admin_message->id}}')"
