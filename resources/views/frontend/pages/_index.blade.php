                      ...
                      <div class="row">
                        @foreach($pages as $Page)
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title">#Code {{$Page->id}}</h5>
                                        <h6 class="card-subtitle mb-2 text-muted">Card subtitle</h6>
                                        <p class="card-text">
                                            Some quick example text to build on the card title and make
                                            up the bulk of the cards content.</p>
                                        <a class="btn btn-primary" href="{{ url('pages/'.$Page->id) }}" class="card-link">view</a>
                                        <a class="btn btn-success" href="{{ url('pages/'.$Page->id.'/edit') }}" class="card-link">update</a>

                                       <form id="{{$Page->id}}" method="POST"
                                       action="{{url('pages/'.$Page->id)}}"
                                       style="display:inline">
                                        <input name="_method" type="hidden" value="DELETE">
                                        <input name="_token" type="hidden" value="{{csrf_token()}}">
                                        <button type="button" onclick="return deleteItem('{{$Page->id}}')"
