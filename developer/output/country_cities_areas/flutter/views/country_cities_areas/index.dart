import '../../Controllers/Country_cities_areaController.dart';
import 'package:flutter/cupertino.dart';
import '../../helpers/LoaderDialog.dart';
import '../../helpers/ToastHelper.dart';
import 'package:framework_01_6/helpers/LanguageHelper.dart' as LanguageHelper;
import '../../lang/ar/country_cities_areas.dart' as messages_ar;
import '../../lang/en/country_cities_areas.dart' as messages_en;
import '../../main.dart';
...
class Country_cities_areasIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _Country_cities_areasIndexState();
  }
}

class _Country_cities_areasIndexState extends State<Country_cities_areasIndex>{
  ...
  String viewType = "ListView";
    ...
    int pagesCount = 1 ;
    int newPage = 2;
  Country_cities_areaController _Country_cities_areaController = new Country_cities_areaController();
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _Country_cities_areaController.index(1,searchText).whenComplete((){
              if(_Country_cities_areaController.status == true){
                setState(() {
                  data = _Country_cities_areaController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _Country_cities_areaController.data["data"]["last_page"];
                  }
                });
              }else{
                ShowToast("warning",_Country_cities_areaController.message);
              }
            });
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _Country_cities_areaController.index(newPage,searchText).whenComplete((){
          if(_Country_cities_areaController.status == true){
            setState(() {
              data.addAll(_Country_cities_areaController.listData);
              newPage++;
            });
          }
          else{
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("Country_cities_areas") : messages_ar.getTranslation
                                              ("Country_cities_areas"),
