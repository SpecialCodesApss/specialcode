import '../../Controllers/Country_citieController.dart';
import 'package:flutter/cupertino.dart';
import '../../helpers/LoaderDialog.dart';
import '../../helpers/ToastHelper.dart';
import 'package:framework_01_6/helpers/LanguageHelper.dart' as LanguageHelper;
import '../../lang/ar/country_cities.dart' as messages_ar;
import '../../lang/en/country_cities.dart' as messages_en;
import '../../main.dart';
...
class Country_citiesIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _Country_citiesIndexState();
  }
}

class _Country_citiesIndexState extends State<Country_citiesIndex>{
  ...
  String viewType = "ListView";
    ...
    int pagesCount = 1 ;
    int newPage = 2;
  Country_citieController _Country_citieController = new Country_citieController();
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _Country_citieController.index(1,searchText).whenComplete((){
              if(_Country_citieController.status == true){
                setState(() {
                  data = _Country_citieController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _Country_citieController.data["data"]["last_page"];
                  }
                });
              }else{
                ShowToast("warning",_Country_citieController.message);
              }
            });
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _Country_citieController.index(newPage,searchText).whenComplete((){
          if(_Country_citieController.status == true){
            setState(() {
              data.addAll(_Country_citieController.listData);
              newPage++;
            });
          }
          else{
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("Country_cities") : messages_ar.getTranslation
                                              ("Country_cities"),
