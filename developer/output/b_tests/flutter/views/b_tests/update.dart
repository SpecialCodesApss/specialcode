...
class B_testsUpdate extends StatefulWidget {

  final int id;
  B_testsUpdate(this.id, {Key key}) : super(key: key);
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _B_testsUpdateState();
  }
}

class _B_testsUpdateState extends State<B_testsUpdate>{
  ...
  _onPressedUpdate(){
    setState(() {
      ...
      if(_users_idsController.text.trim().isNotEmpty&&_pages_idController.text.trim().isNotEmpty&&_table_idsController.text.trim().isNotEmpty&&_page_htmlController.text.trim().isNotEmpty&&_test_2Controller.text.trim().isNotEmpty&&_emailController.text.trim().isNotEmpty&&_typeController.text.trim().isNotEmpty){
        _B_testController.update(widget.id,_users_idsController.text.trim(),int.parse(_pages_idController.text.trim()),_table_idsController.text.trim(),_page_htmlController.text.trim(),_test_2Controller.text.trim(),_emailController.text.trim(),image,_typeController.text.trim()).whenComplete((){
  ...
  read() async {
    ...
    _B_testController.view(widget.id).whenComplete((){
      if(_B_testController.status == true){
        setState(() {
          data=_B_testController.data;_users_idsController.text=_B_testController.data["data"]["users_ids"].toString() ?? "";_pages_idController.text=_B_testController.data["data"]["pages_id"].toString() ?? "";_table_idsController.text=_B_testController.data["data"]["table_ids"].toString() ?? "";_page_htmlController.text=_B_testController.data["data"]["page_html"].toString() ?? "";_test_2Controller.text=_B_testController.data["data"]["test_2"].toString() ?? "";_emailController.text=_B_testController.data["data"]["email"].toString() ?? "";_typeController.text=_B_testController.data["data"]["type"].toString() ?? "";
        });
