import '../../Controllers/CurrencieController.dart';
import 'package:flutter/cupertino.dart';
import '../../helpers/LoaderDialog.dart';
import '../../helpers/ToastHelper.dart';
import 'package:framework_01_5/helpers/LanguageHelper.dart' as LanguageHelper;
import '../../lang/ar/currencies.dart' as messages_ar;
import '../../lang/en/currencies.dart' as messages_en;
import '../../main.dart';
...
class CurrenciesIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _CurrenciesIndexState();
  }
}

class _CurrenciesIndexState extends State<CurrenciesIndex>{
  ...
  String viewType = "ListView";
    ...
    int pagesCount = 1 ;
    int newPage = 2;
  CurrencieController _CurrencieController = new CurrencieController();
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _CurrencieController.index(1,searchText).whenComplete((){
              if(_CurrencieController.status == true){
                setState(() {
                  data = _CurrencieController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _CurrencieController.data["data"]["last_page"];
                  }
                });
              }else{
                ShowToast("warning",_CurrencieController.message);
              }
            });
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _CurrencieController.index(newPage,searchText).whenComplete((){
          if(_CurrencieController.status == true){
            setState(() {
              data.addAll(_CurrencieController.listData);
              newPage++;
            });
          }
          else{
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("Currencies") : messages_ar.getTranslation
                                              ("Currencies"),
