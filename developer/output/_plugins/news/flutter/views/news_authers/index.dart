...
import '../../Views/news_authers/view.dart';


class News_authersIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _News_authersIndexState();
  }
}

class _News_authersIndexState extends State<News_authersIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _News_autherController.index(1,searchText).whenComplete((){
              if(_News_autherController.status == true){
                setState(() {
                  data = _News_autherController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _News_autherController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _News_autherController.index(newPage,searchText).whenComplete((){
          if(_News_autherController.status == true){
            setState(() {
              data.addAll(_News_autherController.listData);
              newPage++;
            });
          }
          else{
                      ...
                      child:InkWell(
                        onTap: () {
                          Navigator.of(context).push(
                              MaterialPageRoute(builder: (context) => News_authersView(data[index]["id"]))
                          );
                        },
                        child: GridTile(
                          child: new Card(
                                    ...
                                    child:  Column(
                                      ...
                                      children: <Widget>[
                                        FadeInImage.assetNetwork(
                                          ...
                                          image:"http://192.168.0.101/framework/"+data[index]["profile_image"] ,
                                          width: 40.0 ,
                                          height: 40.0 ,
                                        ),
                                        Padding(
