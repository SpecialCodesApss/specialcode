...
import '../../Views/news_comments/store.dart';

class News_commentsIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _News_commentsIndexState();
  }
}

class _News_commentsIndexState extends State<News_commentsIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _News_commentController.index(1,searchText).whenComplete((){
              if(_News_commentController.status == true){
                setState(() {
                  data = _News_commentController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _News_commentController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _News_commentController.index(newPage,searchText).whenComplete((){
          if(_News_commentController.status == true){
            setState(() {
              data.addAll(_News_commentController.listData);
              newPage++;
            });
          }
          else{
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("News_comments") : messages_ar.getTranslation
                              ("News_comments"),
    ...
    child: AnimationLimiter(
        child: SmartRefresher(
                                    ...
                                    child:  Column(
                                      ...
                                      children: <Widget>[
                                        FadeInImage.assetNetwork(
                                          ...
                                          image:"http://192.168.1.101/framework/"+data[index]["id"] ,
                                          width: 40.0 ,
                                          height: 40.0 ,
                                        ),
                                        Padding(
                            ...
                            child: InkWell(
                              ...
                              child: Container(
                                  ...
                                  child:
                                      FadeInImage.assetNetwork(
                                        placeholder:"assets/images/noimage.png" ,
                                        image: "http://192.168.0.101/framework/"+data[index]["comment_text"],
                                        width: 40.0 ,
                                        height: 40.0 ,
                                      )
                                      ),
          ...
          floatingActionButton: ViewAddButton==true ? FloatingActionButton(
            ...
            MaterialPageRoute(builder: (context) => News_commentsStore())
            );
         }) : null ,
        );
  }
