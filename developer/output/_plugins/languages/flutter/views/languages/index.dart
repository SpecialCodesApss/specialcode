...
import '../../lang/ar/languages.dart' as messages_ar;
import '../../lang/en/languages.dart' as messages_en;
import '../../main.dart';
...
class LanguagesIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _LanguagesIndexState();
  }
}

class _LanguagesIndexState extends State<LanguagesIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _LanguageController.index(1,searchText).whenComplete((){
              if(_LanguageController.status == true){
                setState(() {
                  data = _LanguageController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _LanguageController.data["data"]["last_page"];
                  }
                });
              }else{
                ShowToast("warning",_LanguageController.message);
              }
            });
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _LanguageController.index(newPage,searchText).whenComplete((){
          if(_LanguageController.status == true){
            setState(() {
              data.addAll(_LanguageController.listData);
              newPage++;
            });
          }
          else{
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("Languages") : messages_ar.getTranslation
                                              ("Languages"),
                                    ...
                                    children: <Widget>[
                                      ...
                                      Row(
                                        ...
                                        children: <Widget>[Padding(
                                            padding: EdgeInsets.only(right: 10.0 , left: 10.0 , top: 0.0) ,
                                            child: Text(
                                               language =="en" ? messages_en.getTranslation("SARcurrency") : messages_ar.getTranslation("SARcurrency")+data[index]["name_ar"].toString() ,
