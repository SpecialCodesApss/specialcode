...
import '../../Views/our_services/view.dart';

class Our_servicesIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _Our_servicesIndexState();
  }
}

class _Our_servicesIndexState extends State<Our_servicesIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _Our_serviceController.index(1,searchText).whenComplete((){
              if(_Our_serviceController.status == true){
                setState(() {
                  data = _Our_serviceController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _Our_serviceController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _Our_serviceController.index(newPage,searchText).whenComplete((){
          if(_Our_serviceController.status == true){
            setState(() {
              data.addAll(_Our_serviceController.listData);
              newPage++;
            });
          }
          else{
                      ...
                      child:InkWell(
                        onTap: () {
                          Navigator.of(context).push(
                              MaterialPageRoute(builder: (context) => Our_servicesView(data[index]["id"]))
                          );
                        },
                        child: GridTile(
                          child: new Card(
                                    ...
                                    child:  Column(
                                      ...
                                      children: <Widget>[
                                        FadeInImage.assetNetwork(
                                          ...
                                          image:"http://192.168.0.101/framework/"+data[index]["image"] ,
                                          width: 40.0 ,
                                          height: 40.0 ,
                                        ),
                                        Padding(
