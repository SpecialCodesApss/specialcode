...
import '../../Views/news_favorites/store.dart';

class News_favoritesIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _News_favoritesIndexState();
  }
}

class _News_favoritesIndexState extends State<News_favoritesIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _News_favoriteController.index(1,searchText).whenComplete((){
              if(_News_favoriteController.status == true){
                setState(() {
                  data = _News_favoriteController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _News_favoriteController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _News_favoriteController.index(newPage,searchText).whenComplete((){
          if(_News_favoriteController.status == true){
            setState(() {
              data.addAll(_News_favoriteController.listData);
              newPage++;
            });
          }
          else{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: ViewSearchBar==true ?
            SearchAppBar(
              ...
              mainAppBar: AppBar(
                title: Text(
                language =="en" ? messages_en.getTranslation("News_favorites") : messages_ar.getTranslation
                                              ("News_favorites"),
    ...
    : Column(
          ...
          floatingActionButton: ViewAddButton==true ? FloatingActionButton(
            ...
            MaterialPageRoute(builder: (context) => News_favoritesStore())
            );
         }) : null ,
        );
  }
