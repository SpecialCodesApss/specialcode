...
class _NewsIndexState extends State<NewsIndex>{
  ...
  RefreshController _refreshController =
    ...
    void _onRefresh() async {
      ...
      //get list data
            _NewsController.index(1,searchText).whenComplete((){
              if(_NewsController.status == true){
                setState(() {
                  data = _NewsController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _NewsController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _NewsController.index(newPage,searchText).whenComplete((){
          if(_NewsController.status == true){
            setState(() {
              data.addAll(_NewsController.listData);
              newPage++;
            });
          }
          else{
                      ...
                      child:InkWell(
                        onTap: () {
                          Navigator.of(context).push(
                              MaterialPageRoute(builder: (context) => NewsView(data[index]["id"]))
                          );
                        },
                        child: GridTile(
                          child: new Card(
                                    ...
                                    child:  Column(
                                      ...
                                      children: <Widget>[
                                        FadeInImage.assetNetwork(
                                          ...
                                          image:"http://192.168.1.4/framework/"+data[index]["image"] ,
                                          width: 40.0 ,
                                          height: 40.0 ,
                                        ),
                                        Padding(
