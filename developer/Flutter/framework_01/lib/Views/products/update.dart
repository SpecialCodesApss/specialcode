...
class ProductsUpdate extends StatefulWidget {
  ...
  final int id;
  ProductsUpdate(this.id, {Key key}) : super(key: key);
  @override
...
class _ProductsUpdateState extends State<ProductsUpdate>{
  ...
  _onPressedUpdate(){
    setState(() {
      ...
      if(_name_arController.text.trim().isNotEmpty&&_name_enController.text.trim().isNotEmpty&&_description_arController.text.trim().isNotEmpty&&_description_enController.text.trim().isNotEmpty&&_html_text_arController.text.trim().isNotEmpty&&_html_text_enController.text.trim().isNotEmpty&&_activeController.text.trim().isNotEmpty&&_user_idController.text.trim().isNotEmpty){
        _ProductController.update(widget.id,_name_arController.text.trim(),_name_enController.text.trim(),image,_description_arController.text.trim(),_description_enController.text.trim(),_html_text_arController.text.trim(),_html_text_enController.text.trim(),_activeController.text.trim(),_user_idController.text.trim()).whenComplete((){
  ...
  read() async {
    ...
    _ProductController.view(widget.id).whenComplete((){
      if(_ProductController.status == true){
        setState(() {
          data=_ProductController.data;_name_arController.text=_ProductController.data["data"]["name_ar"].toString() ?? "";_name_enController.text=_ProductController.data["data"]["name_en"].toString() ?? "";_description_arController.text=_ProductController.data["data"]["description_ar"].toString() ?? "";_description_enController.text=_ProductController.data["data"]["description_en"].toString() ?? "";_html_text_arController.text=_ProductController.data["data"]["html_text_ar"].toString() ?? "";_html_text_enController.text=_ProductController.data["data"]["html_text_en"].toString() ?? "";_activeController.text=_ProductController.data["data"]["active"].toString() ?? "";_user_idController.text=_ProductController.data["data"]["user_id"].toString() ?? "";
        });
      ...
      body: Container(
              ...
              padding: EdgeInsets.all(10.0),
                    ...
                    FadeInImage.assetNetwork(
                                    ...
                                    image: data != null ? "http://192.168.1.101/engineer/"+data["data"]["image"] : "assets/images/noimage.png" ,
                                  ),
                                  image == null
                                      ? Text(
