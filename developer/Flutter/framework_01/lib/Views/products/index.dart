...
import '../../Views/products/store.dart';

class ProductsIndex extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _ProductsIndexState();
  }
}
...
class _ProductsIndexState extends State<ProductsIndex>{
  ...
  RefreshController _refreshController =
      ...
      //get list data
            _ProductController.index(1,searchText).whenComplete((){
              if(_ProductController.status == true){
                setState(() {
                  data = _ProductController.listData;
                  if(data.length > 0){
                    isNoListData=false;
                    pagesCount = _ProductController.data["data"]["last_page"];
                  }
                });
              }else{
    ...
    void _onLoading() async {
      ...
      if (mounted)
        _ProductController.index(newPage,searchText).whenComplete((){
          if(_ProductController.status == true){
            setState(() {
              data.addAll(_ProductController.listData);
              newPage++;
            });
          }
          else{
    ...
    child: AnimationLimiter(
        child: SmartRefresher(
          ...
          floatingActionButton: ViewAddButton==true ? FloatingActionButton(
            ...
            onPressed: () {
            Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => ProductsStore())
            );
         }) : null ,
        );
  }
